...
/**
 * Tehtävänanto: https://www.codewars.com/kata/56a32dd6e4f4748cc3000006
 * Kertoo kaupunkien sademäärien keskiarvon & varianssin. Sademäärät ovat merkkijonona, joissa eri kaupunkien
...
public class Rainfall {
	...
	public static void main(String[] args) {
		...
		System.out.println(variance("Väärä kaupunki", data));
	}

	/**
	 * Palauttaa sademäärän keskiarvon haetulle kaupungille
	 * @param town kaupunki, jonka sademäärä tarkastellaan
	 * @param strng ns. tietokanta, josta haetaan kaupunki
	 * @return keskimääräinen sademäärä kaupungille
	...
	public static double mean (String town, String strng) throws IllegalArgumentException {
		if(town == null || !strng.contains(town) || town.isEmpty()) throw new IllegalArgumentException ("Invalid town");
		String[] towns = strng.split("\n");
		String townData = "";
		for(String t:towns) if(t.contains((town) + ":")) townData = t;
		if(townData.equals("")) return -1;
		townData = townData.replaceAll(town + ":", "");
		String[] months = townData.split(",");
		for(int i = 0; i < months.length; i++) {
			months[i] = months[i].replaceAll("[^\\d.]", "");
		}
		double result = 0;
		for(String m:months) result += Double.parseDouble(m);
		return result /= months.length;
	}
	
	/**
	 * Palauttaa sademäärän varianssin haetulle kaupungille.
	 * @param town kaupunki, jonka sademäärä tarkastellaan
	 * @param strng ns. tietokanta, josta haetaan kaupunki
	 * @return sademäärän varianssi kaupungille
	 * @throws IllegalArgumentException virheilmoituksia varten
	 */
